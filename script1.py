...
# Dictionary to store the media link mappings
link_to_media = {}
...
async def start(update: Update, context: CallbackContext):
    ...
    if start_param and start_param in link_to_media:
        media_data = link_to_media[start_param]
...
async def handle_media(update: Update, context: CallbackContext):
    ...
    if media_type:
        ...
        # Store the media type, media file_id, and caption
        link_to_media[unique_code] = {"type": media_type, "media": media, "caption": caption}
...
async def list_links(update: Update, context: CallbackContext):
    ...
    if not link_to_media:
        await update.message.reply_text("No media links have been created yet.")
        return
    
    # Create the list of links with media type
    links = []
    for idx, (code, media_data) in enumerate(link_to_media.items(), 1):
        media_type = media_data['type']
        start_link = f"https://t.me/{(await context.bot.get_me()).username}?start={code}"
